...
Capsule::Capsule(Player* player, Game* g, PlayState* play, Texture* texture, Vector2D posIni, string tag) : Enemy(g, play, texture, posIni, tag)
{
	_attackTime = 2000;
	_life = 300;
	_speed = -8;
...
void Capsule::update(const double& deltaTime)
{
	...
	if (_spawning)
	{
		...
		enemySpawn(new MeleeEnemy(_game, _playstate, _game->getTexture("EnemyMelee"), Vector2D(enemyPos.x * 8 - 30/*Numero a ajustar dependiendo del sprite*/, enemyPos.y * 8 - 30/*Numero a ajustar dependiendo del sprite*/), "Enemy"));
		destroy();
	}
}
