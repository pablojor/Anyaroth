...
void Map::createObjects()
{
	for (int i = 0; i < _objectLayers.size(); i++)
	{
		...
		for (int j = 0; j < objectData.size(); j++)
		{
			...
			{
				_objects->addChild(new BomberEnemy(_game, _player, Vector2D(pos.getX(), pos.getY() - TILES_SIZE * 2), _explosivePool));
			}
			else if (name == "Spawner")
			{
				_objects->addChild(new SpawnerEnemy(_game, _player, Vector2D(pos.getX(), pos.getY() - TILES_SIZE * 2)));
			}
			...
			{
				_boss1 = (new Boss1(_game, _player, Vector2D(pos.getX(), pos.getY()), _bulletPool, _explosivePool, _bouncingPool));
