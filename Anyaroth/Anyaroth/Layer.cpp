...
Layer::Layer(string name, Texture* t, string filename, b2World* world) : GameComponent(world), _tileset(t)
{
	_tilemap.clear();
	fstream file;
	file.open(filename);
		...
		if (n != "}") {
			...
			for (int y = 0; y < h; y++)
			{
				for (int x = 0; x < w; x++)
				{
					...
					if (temp >= 0)
					{
						Tile* tile = new Tile(x * TILES_W, y * TILES_H, (temp / t->getNumCols()), temp % t->getNumCols(), _tileset, getWorld());
